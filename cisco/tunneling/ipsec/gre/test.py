...
def main():
  ...
  iosv_0.execs([
    [
      f"interface {ini.iosv_0.tunnel0.name}",
      #f"tunnel mode gre",
      f"ip address {ini.iosv_0.tunnel0.ip_addr.ip} {ini.iosv_0.tunnel0.ip_addr.netmask}",
      f"tunnel source {ini.iosv_0.g0_0.ip_addr.ip}",
      f"tunnel destination {ini.iosv_2.g0_0.ip_addr.ip}",
    ],
  ])

  iosv_2.execs([
    [
      f"interface {ini.iosv_2.tunnel0.name}",
      #f"tunnel mode gre",
      f"ip address {ini.iosv_2.tunnel0.ip_addr.ip} {ini.iosv_2.tunnel0.ip_addr.netmask}",
      f"tunnel source {ini.iosv_2.g0_0.ip_addr.ip}",
      f"tunnel destination {ini.iosv_0.g0_0.ip_addr.ip}",
    ],
  ])

  # dynamic(ospf) routing(tunnel and external-network)
  iosv_0.execs([
    [
      f"router ospf {ini.ospf_process_id}",
      f"network {ini.iosv_0.g0_1.ip_addr.ip} {ini.iosv_0.g0_1.ip_addr.hostmask} area 0",
      f"network {ini.iosv_0.tunnel0.ip_addr.ip} 0.0.0.0 area 0",
    ],
  ])
  ...
  iosv_2.execs([
    [
      f"router ospf {ini.ospf_process_id}",
      f"network {ini.iosv_2.g0_1.ip_addr.ip} {ini.iosv_2.g0_1.ip_addr.hostmask} area 0",
      f"network {ini.iosv_2.tunnel0.ip_addr.ip} 0.0.0.0 area 0",
    ],
  ])
